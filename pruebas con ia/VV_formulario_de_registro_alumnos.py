...
class PantallaDeFormularioNuevoRegistroAlumnos(QWidget, Ui_FormularioNuevoRegistroAlumnos):
    def __init__(self, stacked_widget):
        ...
        self.input_primer_nombre.setText("Luis")
        
        self.input_segundo_nombre.setText("Manuel")
        self.input_apellido_paterno.setText("Garcia")
        self.input_apellido_materno.setText("Lopez")
        self.input_cedula.setText("40751989")
        ...
        self.input_cma.setText("12345")
        self.input_imt.setText("56789")
    ...
    def comprobar_representante_registrado(self):
        ...
        cedula_representante = self.input_bucar_por_cedula.text().strip()
        ...
        try:
            ...
            QMessageBox.information(self, "Alerta", f"{e}")
    ...
    def anadir_diagnosticos_alumno_a_lista(self):
        ...
        # [id_diagnostico, nombre_diagnostico, medico_tratante, fecha_diagnotico, fecha_vencimiento_certificado, medicacion]
        #        0                  1                 2                3                        4                     5
        ...
        try:
            ...
            if not self.boton_diagnostico.currentIndex() == 0 and not self.input_otro_diagnostico.text().strip():
                ...
                if errores_info_clinica_alumno:
                    self.mostrar_errores_antes_de_guardar(errores_info_clinica_alumno, "Informeación clinica del alumno")
    ...
    def guardar_informacion_alumno(self):
        ...
        try:
            ...
            ## Info basica del alumno
            
            primer_nombre = self.input_primer_nombre.text().strip()
            segundo_nombre = self.input_segundo_nombre.text().strip()
            ...
            relacion_con_rep = self.input_relacion_con_representante.text().strip()
            situacion = self.input_situacion.text().strip()
            ...
            cma = self.input_cma.text().strip()
            imt = self.input_imt.text().strip()
            ...
            if errores_primera_info_alumno:
            ...
            elif errores_segunda_info_alumno:
                ...
                # si el representan id esta vacio, quiero que registre un nuevo representan
                # NOTA: el representante_id lo obtenemos en la funcion comprobar_representante_resgistrado
                if self.representante_id == None:
                    
                    cedula_representante = self.input_bucar_por_cedula.text().strip()
                    nombre = self.input_nombre_del_representante.text().strip()
                    apellido = self.input_apellido_del_representante.text().strip()
                    direccion_residencia = self.input_direccion_residencia.text().strip()
                    num_telefono = self.input_numero_de_telefono.text().strip()
                    
                    carga_familiar = None
                    
                    if not self.input_carga_familiar.text().strip():
                        carga_familiar = None
                        
                    else:
                        
                        carga_familiar = self.input_carga_familiar.text().strip()
                        carga_familiar = int(carga_familiar)
                    
                    
                    estado_civil = self.input_estado_civil.text().strip()
            ...
            if errores_info_academica:
                ...
                #######################################################################################################################################33
                        ...
                        # info  cuenta de banco del alumno
                        ...
                        if errores_info_academica:
                            ...
                            self.mostrar_errores_antes_de_guardar(errores_info_academica, "información acádemica")
                            return
                            ...
                            if errores_inscripcion:
                                ...
                                try:
                                    ...
                                    QMessageBox.warning(self, "MODO DEBUG: ERROR", f"{e}")
